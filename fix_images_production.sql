-- ═══════════════════════════════════════════════════════════════════════════
-- APOLLO CMS — FIX IMAGES SQL
-- Chạy file này trên database của apollotech.vn
-- Cập nhật toàn bộ hình ảnh theo đúng nguồn từ demo.apollotech.vn
-- ═══════════════════════════════════════════════════════════════════════════

-- Xóa cache hình ảnh cũ (WordPress URLs) để PHP dùng default mới
DELETE FROM `cms_contents` WHERE `key_name` LIKE '%_img' OR `key_name` LIKE '%img%';

-- ─── TRANG CHỦ (index) ──────────────────────────────────────────────────────

-- Solution preview image (visual-wrap)
...
('index', 'proj_3_img',  'https://apollotech.vn/storage/uploads/2026/01/Hinh-du-an-317x267-12.jpg', 'vi'),
('index', 'proj_4_img',  'https://apollotech.vn/storage/uploads/2026/01/Hinh-du-an-317x267-11.jpg', 'vi'),
('index', 'proj_5_img',  'https://apollotech.vn/storage/uploads/2026/01/Hinh-du-an-317x267-10.jpg', 'vi'),
('index', 'proj_6_img',  'https://apollotech.vn/storage/uploads/2026/01/Hinh-du-an-317x267-09.jpg', 'vi'),
('index', 'proj_7_img',  'https://apollotech.vn/storage/uploads/2026/01/Hinh-du-an-317x267-07.jpg', 'vi'),
('index', 'proj_8_img',  'https://apollotech.vn/storage/uploads/2026/01/Hinh-du-an-317x267-08.jpg', 'vi'),
('index', 'proj_9_img',  'https://apollotech.vn/storage/uploads/2026/01/Hinh-du-an-317x267-06.jpg', 'vi'),
('index', 'proj_10_img', 'https://apollotech.vn/storage/uploads/2026/01/Hinh-du-an-317x267-05.jpg', 'vi'),
('index', 'proj_11_img', 'https://apollotech.vn/storage/uploads/2026/01/Hinh-du-an-317x267-04.jpg', 'vi'),
...
('giai-phap', 'elv_img',    'https://apollotech.vn/storage/uploads/2026/03/69089069_2515089335191113_5587747805164109824_o-69c10f607fa6d.jpg', 'vi'),
...
-- ─── XÓA TẤT CẢ URL ĐẦU CŨ WORDPRESS (wp-content) còn sót ─────────────────
UPDATE `cms_contents` 
SET `value` = REPLACE(`value`, 'https://apollotech.vn/wp-content/uploads/', 'https://apollotech.vn/storage/uploads/')
WHERE `value` LIKE '%wp-content/uploads%' AND (`key_name` LIKE '%img%' OR `key_name` LIKE '%image%');

-- ─── XONG ────────────────────────────────────────────────────────────────────
SELECT 'Image URLs updated successfully' AS result;
SELECT page, key_name, LEFT(value, 80) AS url_preview FROM cms_contents WHERE key_name LIKE '%img%' ORDER BY page, key_name;
