    ...
    const observer = new MutationObserver(() => {
        ...
        if (topbar && !topbar.querySelector('.cms-lang-btn')) {
            ...
            langDiv.innerHTML = `<span class="cms-lang-label">Lang:</span>
                ...
                <button id="cms-btn-bulk-translate" class="cms-lang-btn" style="margin-left:10px; background:#10B981; border-color:#10B981; color:#fff;" title="Dùng Google Dịch trang này sang tiếng Anh, rồi bấm nút này để lưu tĩnh toàn bộ.">🌍 Translate & Save All</button>`;
            topbar.prepend(langDiv);
            
            langDiv.querySelectorAll('.cms-lang-btn:not(#cms-btn-bulk-translate)').forEach(b => {
                b.onclick = () => switchLang(b.dataset.lang);
            });
            updateLangUI();

            // Bulk Translate & Save Logic
            document.getElementById('cms-btn-bulk-translate').addEventListener('click', async () => {
                ...
                if (!confirm(`Sẽ trích xuất ${payloads.length} dòng chữ và lưu vào bộ nhớ ngôn ngữ [${targetLang}].\n\nBạn có chắc chắn muốn tiến hành?`)) return;
                
                try {
                    const btn = document.getElementById('cms-btn-bulk-translate');
                    ...
                    if (data.success) {
                        alert(`✅ Đã lưu thành công ${data.saved_count} mục vào ngôn ngữ: [${targetLang}].\n\nHãy tắt Google Dịch và tải lại trang bằng ngôn ngữ ${targetLang} để kiểm tra thành quả!`);
                    } else {
